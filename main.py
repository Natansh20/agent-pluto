...
def parse_args():
    parser = argparse.ArgumentParser(description="Pluto AI Agent")
    parser.add_argument(
        "--mode",
        choices=["local", "online"],
        default="local",
        help="Backend to use: 'local' (Ollama/qwen) or 'online' (Gemini)",
    )
...
def main():
    ...
    state["mode"] = mode  # injected so query_llm can read it
    ...
    while steps < MAX_STEPS:
        ...
        if isinstance(last_obs, dict):
            ...
            if (
                obs_action == "FILESYSTEM"
                and status in ("success", "exists")
                and op in ("create", "write")
                and path
            ):
            ...
            elif obs_action == "PROCESS" and (
                status == "error" or last_obs.get("returncode", 0) != 0
            ):
        ...
        if llm_output["type"] == "TOOL_CALL":
            ...
            args_dict = llm_output["args"]

            # Loop Guard: Prevent rewriting the same file repeatedly
            if (
                action == "FILESYSTEM"
                and args_dict.get("operation") in ("create", "write")
                and args_dict.get("path") == last_written_file
                and not last_run_failed
            ...
            steps += 1
            result = execute_action(action, args_dict, state)
        ...
        if llm_output["type"] == "RESPONSE":
            ...
            # If there is still an active goal and no tool was used, premature termination
